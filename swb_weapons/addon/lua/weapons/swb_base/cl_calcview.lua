...
function SWEP:CalcView(ply, pos, ang, fov)
	...
	if self.Owner and self.ViewbobEnabled then
	...
	return pos, ang + curviewbob * self.ViewbobIntensity, math.Clamp(fov - self.CurFOVMod, 5, 90)
end
...
function SWEP.CreateMove(move)
	...
	and wep.SWBWeapon
	and wep.dt
	and wep.AimBreathingEnabled
	and wep.dt.State == SWB_AIMING then
		CT = CurTime()
...
function SWEP:AdjustMouseSensitivity()
	if self.RunMouseSensMod and self.dt.State == SWB_RUNNING then
